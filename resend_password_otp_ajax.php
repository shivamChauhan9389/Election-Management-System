...
if (!isset($_SESSION['reset_user_id'])) {
    echo json_encode(['success' => false, 'message' => 'No password reset session found']);
    exit;
}
                ...
                try {
                    ...
                    if ($otp_stmt->execute()) {
                        ...
                        if (send_otp_sms($mobile, $otp)) {
                            $otp_sent = true;
                            log_action("Password reset OTP resent via AJAX", $user_id);
                        } elseif (!empty($email) && send_otp_email($email, $otp)) {
                            $otp_sent = true;
                            log_action("Password reset OTP resent via email via AJAX", $user_id);
                        }
                    ...
                    $conn->rollback();
                    log_action("AJAX password reset OTP resend failed: " . $e->getMessage() . " for user ID: " . $user_id);
